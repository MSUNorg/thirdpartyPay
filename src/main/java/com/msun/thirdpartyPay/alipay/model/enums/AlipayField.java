 ...
 * 支付宝相关字段集
 * 
 * @author zxc Apr 13, 2017 6:39:31 PM
 */
public enum AlipayField {
    ...
    /**
     * 设置请求出错时的通知页面路径(该功能需要联系支付宝开通) 当商户通过该接口发起请求时，如果出现提示报错， 支付宝会根据请求出错时的通知错误码通过异步的方式发送通知给商户。
     */
    ...
    /**
     * 接入极简版wap收银台时支持。当商户请求是来自手机支付宝，在手机支付宝登录后， 有生成登录信息token时，使用该参数传入token将可以实现信任登录收银台，不需要再次登录。
    ...
    /**
     * 卖家支付宝账号对应的支付宝唯一用户号。以2088开头的纯16位数字。 登录时，seller_email和seller_user_id两者必填一个。如果两者都填，以seller_user_id为准。
     */
    ...
    /**
     * 用于商户回传参数 该值不能包含等号、与号等特殊字符。如果用户请求时传递了该参数，则返回给商户时会回传该参数。
     */
    ...
    /**
     * 进行一次即时到账批量退款，都需要提供一个批次号，通过该批次号可以查询这一批次的退款交易记录， 对于每一个合作伙伴，传递的每一个批次号都必须保证唯一性。格式为：退款日期（8位）+流水号（3～24位）。
    ...
    /**
     * 退款请求的明细数据: 1. 数据集格式: 第一笔#第二笔#...#第N笔 2. 单笔格式: 原付款支付宝交易号^退款总金额^退款理由 3. 不支持退分润功能
     */
    ...
    /**
     * 1. 退手续费结果返回格式: 交易号^退款金额^处理结果$退费账号^退费账户ID^退费金额^处理结果 2. 不退手续费结果返回格式： 交易号^退款金额^处理结果 3.
