 ...
 */
package com.msun.thirdpartyPay.support;
...
public class HttpRequest {
    ...
    private static HostnameVerifier getTrustedVerifier() {
        if (TRUSTED_VERIFIER == null) TRUSTED_VERIFIER = new HostnameVerifier() {
    ...
    private static StringBuilder addParamPrefix(final String baseUrl, final StringBuilder result) {
        ...
        else if (queryStart < lastChar && baseUrl.charAt(lastChar) != '&') result.append('&');
        return result;
    }

    private static StringBuilder addParam(final Object key, Object value, final StringBuilder result) {
        if (value != null && value.getClass().isArray()) value = arrayToList(value);
    ...
    public static void setConnectionFactory(final ConnectionFactory connectionFactory) {
        if (connectionFactory == null) CONNECTION_FACTORY = ConnectionFactory.DEFAULT;
        else CONNECTION_FACTORY = connectionFactory;
    }
    ...
    public static class Base64 {
         ...
         * specifying <var>srcOffset</var> and <var>destOffset</var>. This method does not check to make sure your
         * arrays are large enough to accomodate <var>srcOffset</var> + 3 for the <var>source</var> array or
         * <var>destOffset</var> + 4 for the <var>destination</var> array. The actual number of significant bytes in
        ...
        /**
         * Similar to {@link #encodeBytes(byte[], int, int)} but returns a byte array instead of instantiating a String.
        ...
        public static byte[] encodeBytesToBytes(byte[] source, int off, int len) {
            ...
            if (off + len > source.length) throw new IllegalArgumentException(
    ...
    private static List<Object> arrayToList(final Object array) {
        ...
        if (array instanceof int[]) for (int value : (int[]) array)
            result.add(value);
        else if (array instanceof boolean[]) for (boolean value : (boolean[]) array)
            result.add(value);
        else if (array instanceof long[]) for (long value : (long[]) array)
            result.add(value);
        else if (array instanceof float[]) for (float value : (float[]) array)
            result.add(value);
        else if (array instanceof double[]) for (double value : (double[]) array)
            result.add(value);
        else if (array instanceof short[]) for (short value : (short[]) array)
            result.add(value);
        else if (array instanceof byte[]) for (byte value : (byte[]) array)
            result.add(value);
        else if (array instanceof char[]) for (char value : (char[]) array)
            result.add(value);
        return result;
    }
     ...
     * <p>
     * This method ensures the path and query segments of the URL are properly encoded such as ' ' characters being
     * encoded to '%20' or any UTF-8 characters that are non-ASCII. No encoding of URLs is done by default by the
     * {@link HttpRequest} constructors and so if URL encoding is needed this method should be called before calling the
     * {@link HttpRequest} constructor.
     *
     * @param url
     * @return encoded URL
     * @throws HttpRequestException
    ...
    public static String encode(final CharSequence url) throws HttpRequestException {
        ...
        try {
            ...
            if (paramsStart > 0 && paramsStart + 1 < encoded.length()) encoded = encoded.substring(0, paramsStart + 1)
                                                                                 + encoded.substring(paramsStart + 1).replace("+",
                                                                                                                              "%2B");
            return encoded;
     ...
     * <p>
     * Each map entry's key will be a parameter name and the value's {@link Object#toString()} will be the parameter
     * value.
     *
     * @param url
     * @param params
     ...
     * <p>
     * The params argument is interpreted as a sequence of name/value pairs so the given number of params must be
     * divisible by 2.
     *
     * @param url
    ...
    private static String setProperty(final String name, final String value) {
        ...
        if (value != null) action = new PrivilegedAction<String>() {

            public String run() {
                return System.setProperty(name, value);
            }
        };
        else action = new PrivilegedAction<String>() {
    ...
    private HttpURLConnection createConnection() {
        try {
            ...
            if (httpProxyHost != null) connection = CONNECTION_FACTORY.create(url, createProxy());
            else connection = CONNECTION_FACTORY.create(url);
     ...
     * <p>
     * This causes all receive methods to use a {@link GZIPInputStream} when applicable so that higher level streams and
     * readers can read the data uncompressed.
     * <p>
     * Setting this option does not cause any request headers to be set automatically so {@link #acceptGzipEncoding()}
     * should be used in conjunction with this setting to tell the server to gzip the response.
     *
     * @param uncompress
     * @return this request
     */
    ...
    protected ByteArrayOutputStream byteStream() {
        ...
        if (size > 0) return new ByteArrayOutputStream(size);
    ...
    public InputStream stream() throws HttpRequestException {
        ...
        if (code() < HTTP_BAD_REQUEST) try {
            ...
            if (stream == null) try {
                ...
                if (contentLength() > 0) throw new HttpRequestException(e);
                else stream = new ByteArrayInputStream(new byte[0]);
            }
        }
    ...
    /**
     * Get buffered reader to response body using the character set returned from {@link #charset()} and the configured
     * buffer size
     *
     * @see #bufferSize(int)
     * @return reader
    ...
    public HttpRequest headers(final Map<String, String> headers) {
        if (!headers.isEmpty()) for (Entry<String, String> header : headers.entrySet())
            header(header);
        return this;
    }
    ...
    /**
     * Get an integer header value from the response falling back to the given default value if the header is missing or
     * if parsing fails
     *
     * @param name
     * @param defaultValue
    ...
    public String[] headers(final String name) {
        ...
        if (values != null && !values.isEmpty()) return values.toArray(new String[values.size()]);
        else return EMPTY_STRINGS;
    }
        ...
        while (start < end) {
            ...
            if (nameEnd != -1 && nameEnd < end) {
                ...
                if (name.length() > 0) {
                    ...
                    if (length != 0) if (length > 2 && '"' == value.charAt(0) && '"' == value.charAt(length - 1)) params.put(name,
                                                                                                                             value.substring(1,
                                                                                                                                             length - 1));
    ...
    protected String getParam(final String value, final String paramName) {
        ...
        while (start < end) {
            ...
            if (nameEnd != -1 && nameEnd < end && paramName.equals(value.substring(start, nameEnd).trim())) {
                ...
                if (valueLength != 0) if (valueLength > 2 && '"' == paramValue.charAt(0)
                                          && '"' == paramValue.charAt(valueLength - 1)) return paramValue.substring(1,
                                                                                                                    valueLength - 1);
    ...
    public HttpRequest contentType(final String contentType, final String charset) {
        ...
        } else return header(HEADER_CONTENT_TYPE, contentType);
    }
    ...
    public HttpRequest progress(final UploadProgress callback) {
        if (callback == null) progress = UploadProgress.DEFAULT;
    ...
    public HttpRequest send(final byte[] input) throws HttpRequestException {
        if (input != null) incrementTotalSize(input.length);
     ...
     * <p>
     * The pairs specified will be URL-encoded in UTF-8 and sent with the 'application/x-www-form-urlencoded'
     * content-type
     *
     * @param values
     * @return this request
     ...
     * <p>
     * The pair specified will be URL-encoded in UTF-8 and sent with the 'application/x-www-form-urlencoded'
     * content-type
     *
     * @param entry
     * @return this request
    ...
    public HttpRequest form(final Map<?, ?> values, final String charset) throws HttpRequestException {
        if (!values.isEmpty()) for (Entry<?, ?> entry : values.entrySet())
            form(entry, charset);
        return this;
    }
    ...
    public HttpRequest trustAllCerts() throws HttpRequestException {
        ...
        if (connection instanceof HttpsURLConnection) ((HttpsURLConnection) connection).setSSLSocketFactory(getTrustedFactory());
        return this;
    }

    /**
     * Configure HTTPS connection to trust all hosts using a custom {@link HostnameVerifier} that always returns
     * <code>true</code> for each host verified
     * <p>
     * This method does nothing if the current request is not a HTTPS request
     *
     * @return this request
    ...
    public HttpRequest trustAllHosts() {
        final HttpURLConnection connection = getConnection();
        if (connection instanceof HttpsURLConnection) ((HttpsURLConnection) connection).setHostnameVerifier(getTrustedVerifier());
        return this;
    }
    ...
    /**
     * Configure an HTTP proxy on this connection. Use {{@link #proxyBasic(String, String)} if this proxy requires basic
     * authentication.
     *
     * @param proxyHost
     * @param proxyPort
