package com.msun.thirdpartyPay.support.http;
...
import com.msun.thirdpartyPay.support.HttpRequest;
import com.msun.thirdpartyPay.support.exception.HttpException;
 ...
 * HttpRequest Wrapper
 * 
 * @author zxc Apr 13, 2017 6:49:26 PM
 */
public class Http {
    ...
    private String doPost() {
        HttpRequest post = HttpRequest.post(url, params, encode).headers(headers).connectTimeout(connectTimeout).readTimeout(readTimeout).acceptGzipEncoding().uncompress(true);
    ...
    private String doGet() {
        HttpRequest get = HttpRequest.get(url, params, encode).headers(headers).connectTimeout(connectTimeout).readTimeout(readTimeout).acceptGzipEncoding().uncompress(true);
        if (ssl) {
            trustHttps(get);
        }
